...
import { Search, Map, Figma, Play, Grid, Zap, ArrowRight } from 'lucide-react';

import e4 from "../../../assets/e4-1.jpg";


const UIUXDesign = () => {
  const deliverables = [
    { icon: <Search size={20} className="text-cyan-400" />, text: "User research & behavior analysis" },
    { icon: <Map size={20} className="text-purple-400" />, text: "User journey mapping & wireframing" },
    { icon: <Figma size={20} className="text-pink-400" />, text: "High-fidelity UI designs (Figma)" },
    { icon: <Play size={20} className="text-emerald-400" />, text: "Interactive prototypes" },
    { icon: <Grid size={20} className="text-blue-400" />, text: "Design systems & style guides" },
    { icon: <Zap size={20} className="text-amber-400" />, text: "Developer handoff assets" },
  ];

  return (
    <section id='ui-ux' className="bg-[#000000] py-20 px-6 md:py-32 text-slate-300">
      <div className="max-w-6xl mx-auto">
        ...
        <h2 className="text-3xl font-light text-center md:text-5xl leading-tight text-[#e9e7e2]">UI/UX Design</h2>
        </div>
          ...
          <div className="flex flex-col order-2 lg:order-1">
            <div className="max-w-xl mb-12">
              <p className="text-xl md:text-2xl text-slate-400 leading-relaxed font-light">
                Our process at <span className="text-white font-normal">Axstar</span> is centered around users and usability. 
                We create <span className="text-slate-200 font-medium">visually compelling interfaces</span> that 
                balance aesthetics with meaningful digital experiences.
              </p>
            </div>
